...
def getDelayList(file):
    with open(file) as f:
        ...
        delayList = []
        for line in zipped1:
            delayList.append((float(line[10])+float(line[11])+float(line[9]))/3000)
    return delayList
...
def getDelayList2(file):
    with open(file) as f:
        ...
        delayList = []
        for line in zipped1:
            delayList.append((float(line[13])+float(line[14])+float(line[11]))/3000)
    return delayList
...
if __name__ == "__main__":
    ...
    thr_list1=(getDelayList(path))
    thr_list2=(getDelayList2(path))
    snr_list1 = (getsnrList(path,7))
    snr_list2 = (getsnrList(path,8))

    fig,ax=plt.subplots(dpi=500)  
    plt.rc('font',family = 'Times New Roman')
    line4 = ax.plot(time_list, thr_list1,"-x",color='darkblue',label='Move-Latency')
    line3 = ax.plot(time_list, thr_list2,"-x",color='yellowgreen',label='Not move-Latency') 
    ax.set_ylabel('Latency (ms)',color='black',fontsize = 15)
    # ax.set_ylim([0, 1200])
    ...
    #total_line = line1 + line2 + line3 + line4
    total_line = line4 + line3 + line1 + line2
    labs = [l.get_label() for l in total_line]
    ax.legend(total_line,labs,bbox_to_anchor=(0.5, 1.2),loc="upper center", ncol=2,frameon=False,fontsize = 13)
    plt.savefig("SNR_latency.jpg")
