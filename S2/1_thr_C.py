...
if __name__ == "__main__":
    for seed in range(5,155,5): #read which seed --------------------------(5,125,5)
        ...
        FILE = "../Cotag/seed%s/S2/%s/%s/2_overall.csv" %(seed,way,people) #give the path
        list_all.append(getThrList(FILE)) ## get all seed
        sum.append(getAvgList(FILE)) ## get all seed
        GoodRateFILE = "../Cotag/seed%s/S2/%s/%s/0_packet_latency.txt" %(seed,way,people) #give the path
